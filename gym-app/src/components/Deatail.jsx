...
const Deatail = ({ exerciesDeatails }) => {
    const { bodyPart, gifUrl, name, target, equipment } = exerciesDeatails;
    const extraDetail = [
        {
          icon: BodyPartImage,
          name: bodyPart,
    ...
    >
      ...
      <Stack sx={{ gap: { lg: "35px", xs: "20px" } }}>
        ...
        {extraDetail?.map((item,i) => (
          <Stack key={item.name + " " +i} direction="row" gap="24px" alignItems="center">
            <Button
              sx={{
                background: "#FFF2DB",
                borderRadius: "50%",
